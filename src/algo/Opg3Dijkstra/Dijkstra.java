    ...
    private HeapType dijkstraSearch(HeapType current) throws NoPathException {
        ...
        for (String s : tmpArr) {
            if ((visited.get(s) == null || visited.get(s) == false) && !unvisitedContains(s)) {
                // Adding 1 to the time, as each leg transfer is assumed
                double time = current.getTime() + 1 + graph.getTime(s, current.getAirCode());
                unvisitedEdges.add(new HeapType(s, time, current));
            }
        }
