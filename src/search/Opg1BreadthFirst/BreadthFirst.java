package search.Opg1BreadthFirst;
...
public class BreadthFirst {
    ...
    static class minHeapComparator implements Comparator<HeapType> {
        ...
        public int compare(HeapType x, HeapType y) {
            return (int)(x.getJumps() - y.getJumps());
        }
    }

    public BreadthFirst(Graph<String> graph, String start, String destination) {
    ...
    public ArrayList<String> getShortestPath() throws NoPathException {
        ...
        HeapType iterator = breadthSearch(startHeapType);
    ...
    private HeapType breadthSearch(HeapType current) throws NoPathException {
        // 1. Am i done?
        ...
        for (String s : tmpArr) {
            if ((visited.get(s) == null || visited.get(s) == false) && !unvisitedContains(s)) {
                double distance = current.getJumps() + 1;
        ...
        return breadthSearch(selectedUnvisited);
    }
